...
export default function ResultsPage({ data, onAnalyzeNew, isLoading, onMatchJD }) {
  ...
  return (
    <div className="rp" style={{ "--accent": sMeta.color }}>
      ...
      <header className="rp-bar">
        ...
        <input
          ref={fileInputRef} type="file" accept=".pdf,.docx,.jpg,.jpeg,.png"
          onChange={(e) => e.target.files?.[0] && onAnalyzeNew(e.target.files[0])}
          disabled={isLoading}
          hidden
        />
        <div className="rp-bar-action">
          <button className="rp-bar-btn rp-bar-btn--ghost" onClick={() => setIsJDModalOpen(true)} disabled={isLoading}>
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
              <path d="M3 1h6l3 3v9H3V1z" stroke="currentColor" strokeWidth="1.4" strokeLinejoin="round" />
              <path d="M5 6h4M5 8.5h4M5 3.5h2" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" />
            </svg>
            <span>Match Job Description</span>
          </button>
          <span className="rp-bar-tt">
            <Tooltip text="Score this exact resume against a specific job posting you paste in" />
          </span>
        </div>
        <div className="rp-bar-action">
          ...
          <span className="rp-bar-tt">
            <Tooltip text="Analyze a different resume and replace these results" />
          </span>
        </div>
      </header>
      ...
      <JDMatchModal
        ...
        onMatchJD={onMatchJD}
        history={jdHistory}
        onAddToHistory={(item) => setJdHistory((prev) => [item, ...prev].slice(0, 5))}
      />
    </div>
  );
}
